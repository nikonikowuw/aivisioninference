...
type AIVisionTask struct {
	...
	StartDate       datatypes.Date `gorm:"type:date;not null;comment:生效起始日期(来自时间配置)" json:"start_date"`
	EndDate         datatypes.Date `gorm:"type:date;not null;comment:生效结束日期(来自时间配置)" json:"end_date"`
	TimeWindows     datatypes.JSON `gorm:"type:jsonb;not null;comment:每天运行的时间段(来自时间配置)" json:"time_windows"`
