...
func TestEdgeNodeService_Lifecycle(t *testing.T) {
	...
	svc := NewEdgeNodeService(nodeRepo, nodeAlgoRepo, pkgRepo, taskRepo, deviceRepo, smartRecordRepo, nil, jwtManager, fileStorage, nil, nil, nil, nil, NewMemoryHeartbeatStore())
...
func TestEdgeNodeService_HandleHeartbeat_ResumesSuspendedTasks(t *testing.T) {
	...
	svc := NewEdgeNodeService(nodeRepo, nodeAlgoRepo, pkgRepo, taskRepo, deviceRepo, smartRecordRepo, nil, jwtManager, fileStorage2, nil, streamManager, nil, nil, NewMemoryHeartbeatStore())
	ctx := context.Background()
...
func TestEdgeNodeService_buildPresignedURL(t *testing.T) {
	...
	deviceRepo := repository.NewDeviceRepository(db)
	smartRecordRepo := repository.NewSmartRecordRepository(db)
	svc := NewEdgeNodeService(nodeRepo, nodeAlgoRepo, pkgRepo, taskRepo, deviceRepo, smartRecordRepo, nil, jwtManager, fileStorage3, nil, nil, nil, nil, NewMemoryHeartbeatStore())
	ctx := context.Background()
