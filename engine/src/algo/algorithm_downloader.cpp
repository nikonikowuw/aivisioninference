...
void AlgorithmDownloader::DoDeploy(
  ...
  LOG_INFO("[AlgorithmDownloader] Starting deployment of {} (package {})", algo_name, algo_package_id);
  ...
  if (temp_fd == -1) {
    LOG_ERROR("[AlgorithmDownloader] Failed to create temp file");
    algo_mgr->UpdateDeploymentStatus(algo_package_id, algo_name, version,
                                     extract_path, "failed",
                                     "创建临时文件失败");
    return;
  ...
  if (!DownloadFile(download_url, temp_tar)) {
    LOG_ERROR("[AlgorithmDownloader] Failed to download algorithm package");
    algo_mgr->UpdateDeploymentStatus(algo_package_id, algo_name, version,
  ...
  if (!VerifyMD5(temp_tar, expected_md5)) {
    LOG_ERROR("[AlgorithmDownloader] MD5 verification failed");
    algo_mgr->UpdateDeploymentStatus(algo_package_id, algo_name, version,
  ...
  if (!ExtractTarGz(temp_tar, extract_path)) {
    LOG_ERROR("[AlgorithmDownloader] Extraction failed");
    algo_mgr->UpdateDeploymentStatus(algo_package_id, algo_name, version,
  ...
  if (so_path.empty()) {
    LOG_ERROR("[AlgorithmDownloader] nikoniko_detector.so not found in {}", extract_path);
    algo_mgr->UpdateDeploymentStatus(algo_package_id, algo_name, version,
                                     extract_path, "failed",
                                     "未找到算法动态库文件");
    return;
  ...
  LOG_INFO("[AlgorithmDownloader] Found algorithm library at: {}", so_path);

  // 5. Load the algorithm package
  auto instance = algo_mgr->Load(algo_name, version, so_path, "{}");
  if (!instance) {
    LOG_ERROR("[AlgorithmDownloader] Failed to load algorithm library into manager");
    algo_mgr->UpdateDeploymentStatus(algo_package_id, algo_name, version,
                                     extract_path, "failed",
                                     "加载算法动态库失败");
    return;
  ...
  // Success!
  LOG_INFO("[AlgorithmDownloader] Deployment of {} successful!", algo_name);
  algo_mgr->UpdateDeploymentStatus(algo_package_id, algo_name, version,
...
bool AlgorithmDownloader::DownloadFile(const std::string &url,
  ...
  if (res != CURLE_OK) {
    LOG_ERROR("[AlgorithmDownloader] CURL error: {} (code: {}) URL: {}", curl_easy_strerror(res), static_cast<int>(res), url);
    curl_easy_cleanup(curl);
  ...
  if (http_code >= 400) {
    LOG_ERROR("[AlgorithmDownloader] HTTP error code: {} URL: {}", http_code, url);
    curl_easy_cleanup(curl);
    std::filesystem::remove(local_path);
    return false;
  }
...
bool AlgorithmDownloader::ExtractTarGz(const std::string &tar_path,
  ...
  try {
    ...
    LOG_ERROR("[AlgorithmDownloader] Failed to create destination directory: {}", e.what());
    return false;
  }
  ...
  if (posix_spawnp(&pid, "tar", nullptr, nullptr, (char *const *)argv,
                   environ) != 0) {
    LOG_ERROR("[AlgorithmDownloader] posix_spawnp failed");
    return false;
  }

  if (waitpid(pid, &status, 0) == -1) {
    LOG_ERROR("[AlgorithmDownloader] waitpid failed");
    return false;
  }
