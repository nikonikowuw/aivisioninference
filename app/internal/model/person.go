...
type Person struct {
	...
	ImageMD5          string  `gorm:"type:varchar(64);index" json:"image_md5"`
	...
	Groups            []PersonGroup      `gorm:"many2many:person_group_members;" json:"groups,omitempty"`
	Tags              []PersonTag        `gorm:"many2many:person_tag_relations;" json:"tags,omitempty"`
...
type PersonTag struct {
	BaseModel
	TagName  string `gorm:"type:varchar(64);uniqueIndex;not null" json:"tag_name"`
	TagColor string `gorm:"type:varchar(7)" json:"tag_color"`
}

// SortableFields 返回允许排序的字段列表
func (PersonTag) SortableFields() []string {
	return []string{"created_at", "tag_name"}
}
