...
namespace aivision
{
    namespace algo
    {
        ...
        void AlgorithmDownloader::StartDeploy(
            ...
            const std::string& algo_dir
        )
        {
            std::thread t(DoDeploy, algo_mgr, download_url, expected_md5, extract_path, algo_package_id, algo_name, version, algo_dir);
            t.detach();
        }
        ...
        )
        {
            ...
            // 4. Find the .so file
            std::string so_path = extract_path + "/" + algo_name + ".so";
            if (!std::filesystem::exists(so_path))
            {
                // Fallback: search for any .so file
                bool found = false;
                try
                {
                ...
                {
                    std::cerr << "[AlgorithmDownloader] Error scanning extract path: " << e.what() << std::endl;
                }
            ...
            std::cout << "[AlgorithmDownloader] Found algorithm library at: " << so_path << std::endl;
        ...
        bool AlgorithmDownloader::VerifyMD5(const std::string& file_path, const std::string& expected_md5)
        {
        ...
        bool AlgorithmDownloader::ExtractTarGz(const std::string& tar_path, const std::string& dest_dir)
        {
            try
            {
            ...
            {
                std::cerr << "[AlgorithmDownloader] Failed to create destination directory: " << e.what() << std::endl;
                return false;
            }

            pid_t pid;
            const char* argv[] = {"tar", "-xzf", tar_path.c_str(), "-C", dest_dir.c_str(), nullptr};
            int status;

            if (posix_spawnp(&pid, "tar", nullptr, nullptr, (char* const*)argv, environ) != 0) {
