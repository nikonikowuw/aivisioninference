...
bool RKMPPPipeline::Initialize(const std::string& config_json) {
    ...
    auto find_json_bool = [&](const std::string& key, bool def) -> bool {
        ...
        if (val_pos == std::string::npos) return def;
...
bool RKMPPPipeline::Start(const std::string& url) {
    ...
    // 从 SDP 解析编码类型
    int coding_type = MPP_VIDEO_CodingAVC; // H.264 默认
...
bool RKMPPPipeline::EncodeFrameEx(HwBufferPtr frame,
    ...
    // 4. 送帧编码
    ret = static_cast<MppApi*>(mpp_enc_mpi_)->encode_put_frame(
        static_cast<MppCtx>(mpp_enc_ctx_), enc_frame);
    if (ret != MPP_OK) {
    ...
    // 5. 获取编码输出
    ret = static_cast<MppApi*>(mpp_enc_mpi_)->encode_get_packet(
        static_cast<MppCtx>(mpp_enc_ctx_), &enc_packet);
    if (ret != MPP_OK) {
    ...
    if (enc_packet && mpp_packet_get_length(enc_packet) > 0) {
        ...
        if (pkt_len <= size) {
            ...
            desc.codec = (enc_codec_type_ == MPP_VIDEO_CodingHEVC) ?
            ...
            if (desc.is_key_frame) {
                ...
                if (hdr_packet) {
                    ret = static_cast<MppApi*>(mpp_enc_mpi_)->control(
                        static_cast<MppCtx>(mpp_enc_ctx_),
                        MPP_ENC_GET_HDR_SYNC, hdr_packet);
                    if (ret == MPP_OK) {
...
void RKMPPPipeline::DestroyDecoder() {
    if (mpp_dec_ctx_) {
        if (mpp_dec_mpi_) {
            static_cast<MppApi*>(mpp_dec_mpi_)->reset(
                static_cast<MppCtx>(mpp_dec_ctx_));
        }
...
void RKMPPPipeline::DecodeNal(const uint8_t* data, size_t size, int64_t pts) {
    if (!decoder_initialized_ || !mpp_dec_ctx_ || !mpp_dec_mpi_) return;
    ...
    while (max_frames-- > 0) {
        ...
        if (ret == MPP_ERR_TIMEOUT || ret != MPP_OK) break;
        if (!frame) break;
...
bool RKMPPPipeline::RtspSetup() {
    ...
    if (sipos == std::string::npos) sipos = resp.find("session:");
...
bool RKMPPPipeline::RtspPlay() {
    ...
    req << "PLAY " << uri << " RTSP/1.0\r\n"
        << "CSeq: " << rtsp_cseq_++ << "\r\n"
        << "User-Agent: aivision-engine\r\n";
    if (!rtsp_session_.empty()) req << "Session: " << rtsp_session_ << "\r\n";
    req << "\r\n";
    int status = 0;
    std::string resp;
    return RtspSendRequest(req.str()) && RtspReadResponse(status, resp) &&
    ...
    if (mpp_frame_get_info_change(mpp_frame)) {
        ...
        if (dec_frame_group_) {
            ...
            if (mpp_buffer_group_get_internal(&fg, MPP_BUFFER_TYPE_DRM) == MPP_OK) {
                dec_frame_group_ = fg;
                static_cast<MppApi*>(mpp_dec_mpi_)->control(
                    static_cast<MppCtx>(mpp_dec_ctx_),
                    MPP_DEC_SET_EXT_BUF_GROUP, fg);
            }
        }
...
void RKMPPPipeline::DestroyEncoder() {
    if (mpp_enc_ctx_) {
        if (mpp_enc_mpi_) {
            static_cast<MppApi*>(mpp_enc_mpi_)->reset(
                static_cast<MppCtx>(mpp_enc_ctx_));
        }
