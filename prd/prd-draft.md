...
综合以上分析，旷视以算法生态和平台闭环见长，英特灵达以边缘一体机和行业场景落地见长。AIVisionInference 的差异化方向详见下方 4.4.4。
...
- 智能记录作为后台一级菜单展示，不归属于系统管理。
- 用户点击「智能记录」后进入一个统一页面，页面内包含识别记录、告警记录、抓拍记录三个 Tab。
...
智能记录应作为后台一级菜单展示，不归属于系统管理。用户点击「智能记录」后进入统一的智能记录页面，页面内包含三个 Tab：识别记录、告警记录、抓拍记录。

三个 Tab 共用基础筛选能力，包括时间范围、设备、设备分组和分页查询；不同 Tab 根据记录类型展示差异化字段与专属筛选条件。Tab 状态应支持通过 URL query 表达，例如 `type=recognition`、`type=alarm`、`type=capture`，以便刷新、收藏和分享具体记录类型视图。

菜单和 Tab 展示必须受权限控制：用户无任何记录查看权限时隐藏「智能记录」一级菜单；用户仅拥有部分记录查看权限时，仅展示对应 Tab。
...
- 智能记录模块必须作为后台一级菜单展示，不归属于系统管理。
- 智能记录菜单必须进入一个统一页面，页面内包含三个 Tab：识别记录、告警记录、抓拍记录。
...
- 用户无任何记录查看权限时，必须隐藏「智能记录」菜单；用户仅拥有部分记录权限时，仅展示对应 Tab。
...
| 算法包质量不可控 | 第三方 `.so` 崩溃或资源泄漏影响推理进程 | CI/CD Sanitizers 拦截、上传自检、RAII 资源管理、超时控制、崩溃自动恢复。 |
